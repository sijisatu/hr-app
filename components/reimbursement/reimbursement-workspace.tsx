  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[24px] bg-[var(--primary)] px-6 py-6 text-white lg:px-8">
        ...
        <h2 className="mt-5 max-w-3xl text-[26px] font-semibold leading-tight sm:text-[32px]">Claim reimbursement karyawan, approval manager, dan allocation HR sekarang ada di satu modul.</h2>
        <p className="mt-3 max-w-4xl text-[14px] leading-6 text-white/80">Employee submit claim dengan receipt, manager review, lalu HR finalisasi dan memotong balance entitlement yang sudah diatur per karyawan.</p>
      </section>
      ...
      {isEmployeeSurface ? (
        <>
          <section className="grid gap-4 lg:grid-cols-3">
            {[{ key: "history", label: "My Claim Applications", value: myHistory.length, note: "Riwayat reimbursement yang pernah dikirim." }, { key: "entitlements", label: "My Claim Entitlements", value: myClaimTypes.length, note: "Claim type aktif yang di-assign HR." }, { key: "drafts", label: "My Draft Claim", value: myDrafts.length, note: "Draft reimbursement yang belum disubmit." }].map((item) => (
          ...
          <section className="page-card p-5 sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <p className="section-title text-[24px] font-semibold text-[var(--primary)]">Pengajuan Reimbursement</p>
                <p className="mt-2 max-w-2xl text-[14px] leading-6 text-[var(--text-muted)]">Designation otomatis dari akun kamu. Pilih claim type, isi receipt date, amount, remarks, lalu upload dokumen receipt.</p>
              </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              ...
              <div className="panel-muted p-4"><p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">Claim Types</p><p className="mt-2 text-[15px] font-semibold text-[var(--primary)]">{myClaimTypes.length}</p><p className="mt-1 text-[13px] text-[var(--text-muted)]">Ditentukan HR per karyawan.</p></div>
              <div className="panel-muted p-4"><p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">Receipt</p><p className="mt-2 text-[15px] font-semibold text-[var(--primary)]">Required on submit</p><p className="mt-1 text-[13px] text-[var(--text-muted)]">Draft boleh disimpan dulu.</p></div>
            </div>
          </section>
        </>
      ) : null}
        ...
        <>
          <section className="page-card overflow-hidden p-0">
            <div className="flex flex-col gap-4 border-b border-[var(--border)] px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
              <div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">All Employee Reimbursements</p><p className="mt-2 text-[14px] text-[var(--text-muted)]">Pantau semua reimbursement dan proses queue HR dari sini.</p></div>
          ...
          <section className="page-card p-5 sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">Claim Type Allocation</p><p className="mt-2 max-w-3xl text-[14px] leading-6 text-[var(--text-muted)]">HR menentukan reimbursement apa saja yang didapat tiap karyawan. Preset di bawah ini mengikuti praktik reimbursement yang umum di perusahaan Indonesia.</p></div>
...
function RequestTable({ title, rows }: { title: string; rows: ReimbursementRequestRecord[] }) {
  return (
    <section className="page-card overflow-hidden p-0">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-6 py-5"><div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">{title}</p><p className="mt-2 text-[14px] text-[var(--text-muted)]">Riwayat reimbursement yang pernah disubmit.</p></div><div className="rounded-[12px] bg-[var(--panel-alt)] px-4 py-3 text-[13px] text-[var(--text-muted)]">{rows.length} claims</div></div>
...
function EntitlementList({ rows }: { rows: ReimbursementClaimTypeRecord[] }) {
  return <section className="page-card p-5 sm:p-6"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[var(--primary)]" /><div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">My Claim Entitlements</p><p className="mt-2 text-[14px] text-[var(--text-muted)]">Claim type reimbursement yang sudah dialokasikan HR untuk akun kamu.</p></div></div><div className="mt-5 grid gap-4 xl:grid-cols-2">{rows.map((item) => <div key={item.id} className="panel-muted p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[15px] font-semibold text-[var(--text)]">{item.claimType}</p><p className="mt-1 text-[13px] text-[var(--text-muted)]">{item.subType} | {formatReimbursementCategory(item.category)}</p></div><StatusPill tone={item.active ? "success" : "neutral"}>{item.active ? "Active" : "Inactive"}</StatusPill></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><p className="text-[12px] uppercase tracking-[0.08em] text-[var(--text-muted)]">Annual Limit</p><p className="mt-1 text-[15px] font-semibold text-[var(--primary)]">{money(item.annualLimit, item.currency)}</p></div><div><p className="text-[12px] uppercase tracking-[0.08em] text-[var(--text-muted)]">Remaining Balance</p><p className="mt-1 text-[15px] font-semibold text-[var(--primary)]">{money(item.remainingBalance, item.currency)}</p></div></div><p className="mt-3 text-[13px] leading-5 text-[var(--text-muted)]">{item.notes || "Belum ada catatan policy."}</p></div>)}</div></section>;
}

function DraftList({ rows, onEdit }: { rows: ReimbursementRequestRecord[]; onEdit: (draft: ReimbursementRequestRecord) => void }) {
  return <section className="page-card p-5 sm:p-6"><div className="flex items-center gap-3"><FolderPen className="h-5 w-5 text-[var(--primary)]" /><div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">My Draft Claim</p><p className="mt-2 text-[14px] text-[var(--text-muted)]">Draft reimbursement yang belum kamu submit ke manager.</p></div></div><div className="mt-5 grid gap-4 xl:grid-cols-2">{rows.map((item) => <div key={item.id} className="panel-muted p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[15px] font-semibold text-[var(--text)]">{item.claimType}</p><p className="mt-1 text-[13px] text-[var(--text-muted)]">{item.subType} | {item.receiptDate}</p></div><StatusPill tone="neutral">Draft</StatusPill></div><p className="mt-3 text-[14px] text-[var(--text)]">{money(item.amount, item.currency)}</p><p className="mt-2 text-[13px] leading-5 text-[var(--text-muted)]">{item.remarks || "Belum ada remarks."}</p><div className="mt-4"><button className="secondary-button" onClick={() => onEdit(item)}><Pencil className="h-4 w-4" />Continue Draft</button></div></div>)}</div></section>;
}
...
function ApprovalQueue({ rows, pending, onApprove }: { rows: ReimbursementRequestRecord[]; pending: boolean; onApprove: (reimbursementId: string, status: "approved" | "rejected") => void }) {
  return <section className="page-card p-5 sm:p-6"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[var(--primary)]" /><div><p className="section-title text-[24px] font-semibold text-[var(--primary)]">Manager Approval Queue</p><p className="mt-2 text-[14px] text-[var(--text-muted)]">Approve atau reject reimbursement yang diajukan karyawan.</p></div></div><div className="mt-5 grid gap-4 xl:grid-cols-2">{rows.map((item) => <div key={item.id} className="panel-muted p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[15px] font-semibold text-[var(--text)]">{item.employeeName}</p><p className="mt-1 text-[13px] text-[var(--text-muted)]">{item.designation} | {item.claimType} - {item.subType}</p></div><StatusPill tone="warning">Pending Manager</StatusPill></div><p className="mt-3 text-[14px] font-semibold text-[var(--primary)]">{money(item.amount, item.currency)}</p><p className="mt-2 text-[13px] leading-5 text-[var(--text-muted)]">{item.remarks || "Tanpa catatan tambahan."}</p><div className="mt-4 flex flex-col gap-2 sm:flex-row"><button className="secondary-button" onClick={() => onApprove(item.id, "rejected")} disabled={pending}><X className="h-4 w-4" />Reject</button><button className="primary-button" onClick={() => onApprove(item.id, "approved")} disabled={pending}><Check className="h-4 w-4" />Approve</button></div></div>)}</div></section>;
}
      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-[var(--border)] px-6 py-5">
          <div>
            ...
            <p className="mt-2 text-[14px] text-[var(--text-muted)]">Tentukan reimbursement apa yang boleh di-claim oleh karyawan.</p>
          </div>
            ...
            <label className="relative space-y-2 text-[14px] font-medium text-[var(--primary)]">
              ...
              {employeePickerOpen ? (
                <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 max-h-64 overflow-auto rounded-[12px] border border-[var(--border)] bg-white p-2 shadow-lg">
                  ...
                  {filteredEmployees.length === 0 ? <div className="px-3 py-3 text-[13px] text-[var(--text-muted)]">Karyawan tidak ditemukan.</div> : null}
                </div>
              ) : null}
            </label>
